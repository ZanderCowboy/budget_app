...
class SQLHelper {
  ...
  static Future<void> _createTables(sql.Database database) async {
    ...
    batch
      ..execute(_accountsTable)
      ..execute(_fundsTable)
      ..execute(_transactionsTable);
  ...
  static Future<void> _createAccounts(sql.Database database) async {
    ...
    // TODO remove value
    const initialInvestmentBalance = 1.0; //1941.49;
    const initialDebtBalance = 1.0; //89316.05 + 6857.01;
    const initialBankBalance = 1.0; //569.57;

    // Investments
    final investAccount = Account(
      name: investment,
      amount: initialInvestmentBalance,
      type: _investment,
    );

    // Debt
    final debtAccount =
        Account(name: debt, amount: initialDebtBalance, type: _debt);

    // Bank
    final bankAccount =
        Account(name: bank, amount: initialBankBalance, type: _bank);

    batch
      ..insert(_accountsTableName, investAccount.toMap())
      ..insert(_accountsTableName, debtAccount.toMap())
      ..insert(_accountsTableName, bankAccount.toMap());
  ...
  static Future<void> _createFunds(sql.Database database) async {
    ...
    batch
      ..insert(_fundsTableName, chequeFund.toMap())
      ..insert(_fundsTableName, cashFund.toMap())
      ..insert(_fundsTableName, savingsFund.toMap());
  ...
  static Future<Transaction> readTransactions(int id) async {
    final db = await SQLHelper.db();
    final transactionMap = await db.query(
  ...
  static Future<List<Funds>> getAllFunds() async {
    ...
    return rows.map(Funds.fromMap).toList();
  }
